// @ts-nocheck
import { capitalize, isEmpty, isFunction, isUndefined } from 'element-plus/es/utils/index.mjs'
...
class Node {
  ...
  constructor(readonly data: Nullable<CascaderOption>, readonly config: CascaderConfig, readonly parent?: Node, readonly root = false) {
  ...
  setCheckState(checked: boolean) {
    ...
    this.checked =
      this.loaded && this.children.filter(child => !child.isDisabled).every(child => child.loaded && child.checked) && checked
